...
select * from film f ;
select title, length, rental_duration from film where length >= 120 and rental_duration >= 4;
...
select payment_id, amount from payment where payment_id=17510;

-- 문제5번) 영화 카테고리 중에서 ,Sci-Fi  카테고리의  카테고리 번호는 몇번인가요?	14
select * from category ;
select name, category_id from category where name='Sci-Fi';
...
select * from rental;
select count(*) from rental where return_date = "2005-06-03";
...
select * from film;
select film_id, title, description, rental_duration from film where rental_duration >= 7;
